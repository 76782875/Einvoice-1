...
import com.park.einvoice.dao.eih5.EmailDao;
...
public class EmailServiceImpl implements EmailService {
	...
	public String sendEmail(String requestStr) {
		try {
			...
			//从数据库获取发票基本信息
			EmailInvoice emailInvoice = emailDao.selectEmailInvoice(invoiceReqSerialNo);
			//将email地址和pdf文件全路径作为参数，发送邮件
			MailTools.sendMail(email, invoicePdf.toString(), emailInvoice);
			//邮件发送之后删除文件
